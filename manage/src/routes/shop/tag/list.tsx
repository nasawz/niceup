...
import { useTag, useTagById } from '../../../hook/useTag';
import { useTagMuation } from '../../../hook/useTagMuation';
...
export default function TagList(props: ITagListProps) {
  ...
  const { data, error, mutate }: any = useTag(dataParams);
  const {
    data: editData,
    error: editError,
    mutate: editMutate,
  }: any = useTagById(editId);

  const [selectedItems, setSelectedItems] = useState([]);
  const isLoading = !error && !data;
  const isEditLoading = !editError && !editData;

  const { create, update, destroy, state } = useTagMuation();
          ...
          <Switch
            ...
            onChange={() => {
              update(rec._id, { state: state == 0 ? 1 : 0 }, err => {
                if (!err) {
                  // mutate();
                  showToast(`${rec.label}${state == 0 ? '已上架' : '已下架'}`);
                }
              });
            }}
          ></Switch>
      ...
      return (
        ...
        >
          <FormRow
            name="label"
            label="标签"
            messageVariables={{ displayName: '标签' }}
            rules={[{ required: true }]}
          >
            <EuiFieldText />
          </FormRow>
    ...
    flyout = (
      ...
      >
        <EuiFlyoutHeader hasBorder>
          <EuiTitle size="s">
            <h2 id="flyoutSmallTitle">标签分类</h2>
          </EuiTitle>
        </EuiFlyoutHeader>
